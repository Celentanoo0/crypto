...
export default {
  data() {
    return {
      tickerNameInput: "",
      tickers: [{ name: "test", price: "-" }],
      selectedTicker: null,
      coinsToRecommend: [1,2,3,4],
    };
  },

  nonReactData: {
    coins: null, // replace to data
  },
  ...
  created() {
    const cryptoCoins = async () => {
      ...
      this.$options.nonReactData.coins = result.Data;
    }
    cryptoCoins();
  },
  ...
  methods: {
    addNewTicker() {
      ...
      console.log(this.$options.nonReactData.coins)
    },
